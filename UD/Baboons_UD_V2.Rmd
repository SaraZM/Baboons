...
locs <- read.csv("/home/sara/Documentos/Third_Rotation/BaboonsV2/6Days6Baboons")
locs$GMT <- as.POSIXct(locs$GMT) %>%  ymd_hms()
days <- levels(locs$Date)

names(locs) <- c("event.id","GMT","Longitude","Latitude","Deploy_ID","Date" )
```
...
xy <- locs[,c(3,4)]
xy_p1 <-xy[1:118520,]
xy_p2 <-xy[118521:237041,]
#write.csv(xy_p1, "/home/sara/Documentos/Third_Rotation/CTDS/locations_part1.csv", quote=FALSE, row.names = FALSE)
#write.csv(xy_p2, "/home/sara/Documentos/Third_Rotation/CTDS/locations_part2.csv", quote=FALSE, row.names = FALSE)

# In the following step we just read the output from the data and we merge it with the precious one to keep both systems
locsutm <- read.csv("/home/sara/Documentos/Third_Rotation/BaboonsV2/locations_utm2.csv" )
#locs <- data.frame(locs,locsutm)
     intervals <- data.frame(intervals,locsutm)
#write.csv(locs, "/home/sara/Documentos/Third_Rotation/BaboonsV2/UD/locations_utm.csv", quote=FALSE, row.names = FALSE)
```
...
```{r}
loc.stack = stack( intercept, d2trees, d2trail, d2clearing)
names(loc.stack) <- c("intercept","d2trees", "d2trail", "d2clearing" )

grad.stack=stack( d2sleep,d2trees, d2trail, d2clearing)
names(grad.stack) <- c("d2sleep","d2trees", "d2trail", "d2clearing")
...
for (j in 1:6)
for(i in 1:P){
  new_locs <- locs[locs$Date==days[i],]
  my_path <- list(xy=as.matrix(new_locs[,7:8]), t=as.vector(1:nrow(new_locs)))
...
fit=glm(z~d2trees_loc+d2trail_loc+d2clearing_loc+d2sleep_grad+d2trees_grad+d2trail_grad+d2clearing_grad , weights=rep(1/P,nrow(glm.data)),family="poisson",offset=log(tau),data=glm.data)
